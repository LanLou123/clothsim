...
public:
    ...
    void RotateFixup(float deg);
    void RotateFixRight(float deg);
	...
	void enableSphericalCamera()
	{
		enable_spherical = true;
	}

	void disAbleSphericalCamera()
	{
		enable_spherical = false;
	}

	float vertical, horizontal, dis;
	glm::vec3 sphericalTrans = glm::vec3(0,0,0);

	void rotateSphericalUp(float amt);
	void rotateSphericalright(float amt);
	void translateSphericalUp(float amt);
	void translateSphericalRight(float amt);
	void zoomInOut(float amt);
};
